...
Перед стабильным релизом нужны: воспроизводимый dependency lock с hashes, тестовая матрица версий macOS/FFmpeg и отдельная проверка публикуемых примеров на права/персональные данные. Исторический commit импорта `video-use` не был записан; вместо выдуманного hash в `PROVENANCE.md` сохранено документированное сравнение всех релевантных датированных upstream-версий. Formal SBOM обязателен, если будущий релиз начнёт поставлять готовые runtimes или binaries.
